...
void stream_read_clob(OCILobLocator *lobl, FILE *fp)
{
  ...
  OCILobGetLength2(svchp, errhp, lobl, &loblen);
  amtp = loblen;
  ...
  do
  {
      ...
      retval = OCILobRead2(svchp,errhp,lobl,&amtp,NULL,offset,(void *) bufp,
                          (ub8)MAXBUFLEN ,(ub1)OCI_FIRST_PIECE, (void *)0,
      ...
      switch (retval)
      {
         ...
         case OCI_NEED_DATA:           /* there are 2 or more pieces */
             fwrite((void *)bufp, amtp, 1, fp); /* buffer to write */
             break;
         case OCI_ERROR:
